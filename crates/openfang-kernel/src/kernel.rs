...
impl OpenFangKernel {
    ...
    pub fn boot_with_config(mut config: KernelConfig) -> KernelResult<Self> {
        ...
        // Create LLM driver.
        // For the API key, try: 1) explicit api_key_env from config/custom provider,
        // 2) provider_api_keys mapping, 3) auth profiles, 4) convention {PROVIDER}_API_KEY.
        let custom_match = config
            .custom_providers
            .iter()
            .find(|cp| cp.name == config.default_model.provider);
        // Create LLM driver.
        // For the API key, try explicit env first and resolve via mapping/convention as fallback.
        let default_api_key = if !config.default_model.api_key_env.is_empty() {
            std::env::var(&config.default_model.api_key_env).ok()
        } else if let Some(custom) = custom_match.filter(|cp| !cp.api_key_env.is_empty()) {
            std::env::var(&custom.api_key_env).ok()
        } else {
        ...
        match &primary_result {
            ...
            Err(e) => {
                ...
                if let Some(probe) = drivers::detect_available_provider() {
                    let auto_config = DriverConfig {
                        provider: probe.provider.to_string(),
                        api_key: std::env::var(probe.api_key_env).ok(),
                        base_url: config.provider_urls.get(probe.provider).cloned(),
                        api_format: None,
                    };
                    match drivers::create_driver(&auto_config) {
                        Ok(d) => {
                            ...
                            config.default_model.provider = probe.provider.to_string();
                            config.default_model.model = probe.model.to_string();
                            config.default_model.api_key_env = probe.api_key_env.to_string();
                        }
                        Err(e2) => {
                            warn!(provider = %probe.provider, error = %e2, "Auto-detected provider also failed");
                        }
                    }
                }
            }
        ...
        for fb in &config.fallback_providers {
            ...
            } else if let Some(custom) = fb_custom.filter(|cp| !cp.api_key_env.is_empty()) {
                std::env::var(&custom.api_key_env).ok()
            } else {
